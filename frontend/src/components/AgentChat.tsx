...
interface Props {
  onPathwayReady: (pathway: Pathway, convId: string) => void;
...
const PHASE_LABELS: Record<AgentPhase, string> = {
  goal_clarifier: "Goal Clarifier",
  pathway_planner: "Pathway Planner",
  task_quantifier: "Task Quantifier",
};
...
export default function AgentChat({ onPathwayReady, conversationId: initialConvId }: Props) {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (phase === "goal_clarifier" && res.reply.includes("[PROFILE_COMPLETE]")) {
        ...
        setTransitionMsg("Building your personalized learning pathway...");
  ...
  const triggerPathwayGeneration = (cid: string) => {
    setTimeout(async () => {
      try {
        setTransitionMsg("Analyzing modules, breaking down tasks...");
        const f = await api.chat({
          conversation_id: cid,
          agent: "pathway_planner",
          message: "Please generate a learning pathway based on my goals.",
        });
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: f.reply, agent: "pathway_planner" },
        ]);
        if (f.pathway) {
    ...
    <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full p-5">
          ...
          {["goal_clarifier", "pathway_planner", "task_quantifier"].map((p) => {
            ...
            return (
              <div
                ...
                style={{
                  background:
                    active || done ? "var(--color-accent)" : "var(--bg-progress)",
        ...
        {messages.length === 0 && !transitionMsg && (
          <div className="text-center py-20">
            ...
            <p className="text-xs mt-1" style={{ color: "var(--text-tertiary)" }}>
              e.g. "I want to learn C++ for work"
            </p>
          </div>
        )}
      ...
      >
        <input
          ...
          placeholder={
            phase === "goal_clarifier"
              ? "Describe what you want to learn..."
              : 'Ask a question or type "continue"...'
          }
          className="input-field flex-1"
          disabled={loading || !!transitionMsg}
        />
        <button
          type="submit"
          disabled={loading || !input.trim() || !!transitionMsg}
