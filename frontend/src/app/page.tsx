...
export default function HomePage() {
  ...
  const [pathways, setPathways] = useState<QuestSummary[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.listPathways(DEMO_USER).then(setPathways).catch(() => setPathways([])).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="max-w-5xl mx-auto w-full px-5 pt-12 pb-24">
        <div className="mb-12">
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight mb-2" style={{ color: "var(--text-primary)" }}>Your learning paths</h1>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>Turn any skill into a structured, trackable plan.</p>
        </div>
        ...
        <button onClick={() => router.push("/new")} className="card card-hover w-full flex items-center gap-4 py-5 mb-8 cursor-pointer group">
          ...
          <div className="flex-1 text-left">
            <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>New learning path</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--text-secondary)" }}>Tell me what you want to learn, and I will build you a plan.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm font-medium" style={{ color: "var(--text-secondary)" }}>No learning paths yet</p>
            <p className="text-xs mt-1" style={{ color: "var(--text-tertiary)" }}>Tap "New learning path" above to get started.</p>
          </div>
        ) : (
